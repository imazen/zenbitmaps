//! Vendored QOI encoder.
//!
//! Vendored from rapid-qoi v0.6.x (<https://github.com/zakarumych/rapid-qoi>),
//! © zakarumych, licensed MIT OR Apache-2.0.
//!
//! Local modification relative to upstream: the `bytemuck::cast_slice` call was
//! replaced with the safe `<[u8]>::as_chunks::<N>()` (stable since Rust 1.88)
//! to avoid a new direct dependency. The assert guarantees the remainder is
//! empty. No algorithmic changes — encoded output is byte-identical to upstream.
...
impl Qoi {
    ...
    #[inline]
    pub(crate) fn encode(&self, pixels: &[u8], output: &mut [u8]) -> Result<usize, EncodeError> {
    ...
    #[inline]
    // Vendored upstream; the `index_pos as u8` casts are no-ops here but kept to
    // match upstream verbatim.
    #[allow(clippy::unnecessary_cast)]
    pub(crate) fn encode_range<const N: usize>(
        index: &mut [[u8; 4]; 64],
        px_prev: &mut [u8; N],
        run: &mut usize,
        pixels: &[u8],
        ...
        [u8; N]: Pixel,
    {
        ...
        // Safe equivalent of upstream's `bytemuck::cast_slice`: the assert above
        // guarantees the chunked remainder is empty.
        let (mut pixels, rem) = pixels.as_chunks::<N>();
    ...
    #[inline]
    pub(crate) fn encoded_size_limit(&self) -> usize {
    ...
    /// On failure this function returns `Err(err)` with `err` describing cause of the error.
    #[inline]
    pub(crate) fn encode_alloc(&self, pixels: &[u8]) -> Result<Vec<u8>, EncodeError> {
